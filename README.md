...
Quectel 模組支援 NAT 和透傳兩種模式，適用於不同的應用場景：
...
// 配置 NAT 類型 (1=靜態NAT, 2=動態NAT, 3=PAT)
AT+QCFG="nat",1,3

// 配置 NAT 映射表
AT+QNAT=1,"192.168.1.100","203.0.113.10"

// 配置 NAT 埠映射
AT+QNAT=1,"192.168.1.100:8080","203.0.113.10:80"
...
// 配置透傳 IP 範圍
AT+QPASS=1,"192.168.1.0/24"

// 配置透傳埠範圍
AT+QPASS=1,"192.168.1.0/24","80,443,8080-8090"
...
// 配置 DHCP 地址池
AT+QDHCP=1,"192.168.1.100","192.168.1.200"

// 配置 DHCP 租約時間 (秒)
AT+QDHCP=1,"192.168.1.100","192.168.1.200",3600

// 配置 DHCP 選項 (DNS, 閘道)
AT+QDHCP=1,"192.168.1.100","192.168.1.200",3600,"192.168.1.1","8.8.8.8"
...
// 配置 DHCP 客戶端選項
AT+QDHCPC=1,"hostname","device001"
...
```at
// 配置入站防火牆規則
AT+QFIREWALL=1,"IN","192.168.1.100","80","ALLOW"

// 配置出站防火牆規則
AT+QFIREWALL=1,"OUT","0.0.0.0/0","443","ALLOW"

// 配置防火牆預設策略
AT+QFIREWALL=1,"DEFAULT","DROP"
...
```at
// 配置埠轉發
AT+QFORWARD=1,"203.0.113.10:80","192.168.1.100:8080"

// 配置多個埠轉發
AT+QFORWARD=1,"203.0.113.10:80","192.168.1.100:8080"
AT+QFORWARD=2,"203.0.113.10:443","192.168.1.101:8443"
...
// 測試 NAT 連接
AT+QNATTEST="192.168.1.100","8.8.8.8"
...
// 測試 NAT 埠映射
AT+QNATPORT="192.168.1.100:8080"
```
...
Quectel 模組支援 5G_LAN 功能，允許設備接入本地區域網路而非獲得全球 IP 地址：
...
// 配置 5G_LAN 服務類型 (1=企業LAN, 2=家庭LAN, 3=工業LAN)
AT+QCFG="5glan",1,1
...
```at
// 配置 5G_LAN 網路識別碼
AT+QCFG="5glan",1,1,"enterprise_lan_001"

// 配置 5G_LAN 服務區域
AT+QCFG="5glan",1,1,"enterprise_lan_001","zone_001"

// 配置 5G_LAN 服務品質
AT+QCFG="5glan",1,1,"enterprise_lan_001","zone_001",1
...
```at
// 配置 5G_LAN 靜態 IP
AT+QCFG="5glan_ip","192.168.1.100"

// 配置 5G_LAN 子網路遮罩
AT+QCFG="5glan_ip","192.168.1.100","255.255.255.0"

// 配置 5G_LAN 閘道
AT+QCFG="5glan_ip","192.168.1.100","255.255.255.0","192.168.1.1"

// 配置 5G_LAN DNS
AT+QCFG="5glan_ip","192.168.1.100","255.255.255.0","192.168.1.1","192.168.1.1"
...
```at
// 啟用 5G_LAN DHCP 客戶端
AT+QCFG="5glan_dhcp",1

// 配置 5G_LAN DHCP 選項
AT+QCFG="5glan_dhcp",1,"hostname","5g_device_001"

// 配置 5G_LAN DHCP 租約更新
AT+QCFG="5glan_dhcp",1,"hostname","5g_device_001",1
...
###### 5G_LAN 認證配置
```at
// 配置 5G_LAN 認證方式 (1=5G認證, 2=企業認證, 3=混合認證)
AT+QCFG="5glan_auth",1

// 配置 5G_LAN 認證憑證
AT+QCFG="5glan_auth",1,"certificate_path"

// 配置 5G_LAN 認證金鑰
AT+QCFG="5glan_auth",1,"certificate_path","private_key_path"
...
###### 5G_LAN 加密配置
```at
// 配置 5G_LAN 加密算法 (1=AES256, 2=ChaCha20, 3=SM4)
AT+QCFG="5glan_encrypt",1

// 配置 5G_LAN 完整性保護
AT+QCFG="5glan_encrypt",1,1

// 配置 5G_LAN 金鑰更新週期
AT+QCFG="5glan_encrypt",1,1,3600

// 檢查 5G_LAN 加密狀態
AT+QCFG="5glan_encrypt"?
```
...
```at
// 配置本地服務器存取
AT+QCFG="5glan_service","file_server","192.168.1.100","445"

// 配置多個本地服務
AT+QCFG="5glan_service","file_server","192.168.1.100","445"
AT+QCFG="5glan_service","print_server","192.168.1.101","631"
AT+QCFG="5glan_service","db_server","192.168.1.102","3306"

// 檢查本地服務配置
AT+QCFG="5glan_service"?
```

###### 5G_LAN QoS 配置
```at
// 配置 5G_LAN QoS 等級 (1=低延遲, 2=高頻寬, 3=高可靠性)
AT+QCFG="5glan_qos",1

// 配置 5G_LAN 頻寬限制
AT+QCFG="5glan_qos",1,1000000

// 配置 5G_LAN 延遲要求
AT+QCFG="5glan_qos",1,1000000,10
...
```at
// 檢查 5G_LAN 連接狀態
AT+QCFG="5glan_status"?

// 檢查 5G_LAN 網路品質
AT+QCFG="5glan_quality"?

// 檢查 5G_LAN 延遲統計
AT+QCFG="5glan_latency"?

// 檢查 5G_LAN 頻寬使用
AT+QCFG="5glan_bandwidth"?
```
...
// 檢查 5G_LAN 錯誤統計
AT+QCFG="5glan_errors"?

// 檢查 5G_LAN 連接歷史
AT+QCFG="5glan_history"?

// 清除 5G_LAN 統計
AT+QCFG="5glan_stats",0
```
...
```at
// 診斷 5G_LAN 連接
AT+QCFG="5glan_diagnose"?

// 測試 5G_LAN 到本地服務器連接
AT+QCFG="5glan_test","192.168.1.100"

// 檢查 5G_LAN 路由表
AT+QCFG="5glan_route"?

// 檢查 5G_LAN 防火牆規則
AT+QCFG="5glan_firewall"?
```

###### 5G_LAN 配置驗證
```at
// 驗證 5G_LAN 配置
AT+QCFG="5glan_verify"?

// 檢查 5G_LAN 配置衝突
AT+QCFG="5glan_conflict"?

// 測試 5G_LAN 服務存取
AT+QCFG="5glan_service_test","file_server"
...
- 映射表配置不當
**解決方案**:
1. 檢查 NAT 模式: `AT+QCFG="nat"?`
2. 重新配置 NAT 模式: `AT+QCFG="nat",1,3`
3. 檢查 NAT 映射表: `AT+QNAT?`
4. 重新配置映射表: `AT+QNAT=1,"192.168.1.100","203.0.113.10"`
...
**解決方案**:
1. 檢查透傳配置: `AT+QPASS?`
2. 重新配置透傳 IP 範圍: `AT+QPASS=1,"192.168.1.0/24"`
3. 配置透傳埠範圍: `AT+QPASS=1,"192.168.1.0/24","80,443,8080-8090"`
4. 驗證透傳規則

##### NAT 與 DHCP 衝突
**症狀**: IP 地址分配混亂或重複
**原因**:
- NAT 模式下的 DHCP 服務器配置錯誤
- 透傳模式下的 DHCP 客戶端配置錯誤
- 地址池範圍重疊
**解決方案**:
1. 檢查 DHCP 配置: `AT+QDHCP?` 或 `AT+QDHCPC?`
2. 重新配置 DHCP 地址池: `AT+QDHCP=1,"192.168.1.100","192.168.1.200"`
3. 避免地址池重疊
4. 檢查 DHCP 租約狀態
...
- 內部服務未啟動
**解決方案**:
1. 檢查埠轉發配置: `AT+QFORWARD?`
2. 重新配置埠轉發: `AT+QFORWARD=1,"203.0.113.10:80","192.168.1.100:8080"`
3. 檢查防火牆規則: `AT+QFIREWALL?`
4. 確認內部服務狀態
...
- 記憶體使用過高
**解決方案**:
1. 檢查 NAT 統計: `AT+QNATSTATS?`
2. 檢查連接表: `AT+QNATCONN?`
3. 調整連接超時: `AT+QNATTRACK=1,300`
4. 增加最大連接數: `AT+QNATTRACK=1,300,1000`
5. 清除過期連接: `AT+QNATCLEAR`
...
- NAT 日誌未啟用
**解決方案**:
1. 檢查防火牆規則: `AT+QFIREWALL?`
2. 配置預設拒絕策略: `AT+QFIREWALL=1,"DEFAULT","DROP"`
3. 限制埠轉發範圍
4. 啟用 NAT 日誌: `AT+QNATLOG=1`
5. 定期檢查安全日誌

#### 5G_LAN 配置問題

##### 5G_LAN 模式啟用失敗
**症狀**: 無法啟用 5G_LAN 模式或連接失敗
**原因**:
- 模組不支援 5G_LAN 功能
- 5G_LAN 服務未在網路中啟用
- 認證配置錯誤
**解決方案**:
1. 檢查 5G_LAN 支援: `AT+QCFG="5glan"?`
2. 確認網路支援 5G_LAN 服務
3. 檢查認證配置: `AT+QCFG="5glan_auth"?`
4. 重新配置 5G_LAN 模式: `AT+QCFG="5glan",1,1`

##### 5G_LAN IP 地址配置問題
**症狀**: 無法獲得區域網路 IP 地址或 IP 衝突
**原因**:
- 5G_LAN IP 範圍配置錯誤
- DHCP 服務器配置不當
- 靜態 IP 地址衝突
**解決方案**:
1. 檢查 5G_LAN IP 配置: `AT+QCFG="5glan_ip"?`
2. 重新配置靜態 IP: `AT+QCFG="5glan_ip","192.168.1.100","255.255.255.0"`
3. 檢查 DHCP 配置: `AT+QCFG="5glan_dhcp"?`
4. 避免 IP 地址衝突

##### 5G_LAN 認證問題
**症狀**: 5G_LAN 認證失敗或連接被拒絕
**原因**:
- 認證憑證配置錯誤
- 認證金鑰不匹配
- 認證方式設定不當
**解決方案**:
1. 檢查認證配置: `AT+QCFG="5glan_auth"?`
2. 重新配置認證憑證: `AT+QCFG="5glan_auth",1,"certificate_path"`
3. 檢查認證金鑰: `AT+QCFG="5glan_auth",1,"certificate_path","private_key_path"`
4. 驗證認證方式設定
...
- 防火牆阻擋
**解決方案**:
1. 檢查本地服務配置: `AT+QCFG="5glan_service"?`
2. 重新配置服務存取: `AT+QCFG="5glan_service","file_server","192.168.1.100","445"`
3. 檢查路由配置: `AT+QCFG="5glan_route"?`
4. 檢查防火牆規則: `AT+QCFG="5glan_firewall"?`
...
- 網路擁塞
**解決方案**:
1. 檢查 QoS 配置: `AT+QCFG="5glan_qos"?`
2. 調整頻寬限制: `AT+QCFG="5glan_qos",1,1000000`
3. 配置延遲要求: `AT+QCFG="5glan_qos",1,1000000,10`
4. 監控網路品質: `AT+QCFG="5glan_quality"?`

##### 5G_LAN 安全問題
**症狀**: 5G_LAN 連接不安全或資料洩露
**原因**:
- 加密配置不當
- 防火牆規則過於寬鬆
- 存取控制配置錯誤
**解決方案**:
1. 檢查加密配置: `AT+QCFG="5glan_encrypt"?`
2. 配置高強度加密: `AT+QCFG="5glan_encrypt",1,1`
3. 檢查防火牆規則: `AT+QCFG="5glan_firewall"?`
4. 配置存取控制: `AT+QCFG="5glan_acl","192.168.1.100","192.168.1.200","PERMIT"`
...
// 配置 NAT 緩衝區大小
AT+QNATBUFFER=1024  // 1KB 緩衝區

// 配置 NAT 連接表大小
AT+QNATCONN=1000  // 1000 個連接
...
```at
// 配置 NAT 路由表大小
AT+QNATROUTE=100  // 100 條路由

// 配置 NAT 路由更新間隔
AT+QNATROUTE=100,60  // 60 秒更新間隔

// 啟用 NAT 路由快取
AT+QNATROUTE=100,60,1
...
```at
// 配置 NAT 防火牆規則
AT+QFIREWALL=1,"NAT","192.168.1.0/24","ALLOW"

// 配置 NAT 埠掃描防護
AT+QNATSCAN=1,100  // 100 次/分鐘限制

// 配置 NAT 異常檢測
AT+QNATANOMALY=1,1  // 啟用異常檢測
...
```at
// 配置 NAT 負載平衡
AT+QNATLB=1,"round_robin"

// 配置 NAT 健康檢查
AT+QNATLB=1,"round_robin",30  // 30 秒檢查間隔

// 配置 NAT 故障轉移
AT+QNATLB=1,"round_robin",30,1
...
```at
// 配置 NAT 效能監控
AT+QNATMON=1,60  // 60 秒監控間隔

// 配置 NAT 告警閾值
AT+QNATMON=1,60,80  // 80% 使用率告警

// 配置 NAT 日誌記錄
AT+QNATMON=1,60,80,1
...
```at
// 配置 5G_LAN 連接池大小
AT+QCFG="5glan_connpool",10  // 10 個連接

// 配置 5G_LAN 連接超時
AT+QCFG="5glan_timeout",30  // 30 秒超時

// 配置 5G_LAN 連接重用
AT+QCFG="5glan_reuse",1

// 配置 5G_LAN 連接保活
AT+QCFG="5glan_keepalive",60  // 60 秒保活間隔

// 檢查 5G_LAN 連接配置
AT+QCFG="5glan_connpool"?
```

##### 5G_LAN 記憶體優化
```at
// 配置 5G_LAN 緩衝區大小
AT+QCFG="5glan_buffer",2048  // 2KB 緩衝區

// 配置 5G_LAN 快取大小
AT+QCFG="5glan_cache",1024  // 1KB 快取

// 配置 5G_LAN 記憶體池
AT+QCFG="5glan_mempool",512  // 512 個記憶體塊

// 監控 5G_LAN 記憶體使用
AT+QCFG="5glan_memory"?
```

##### 5G_LAN 路由優化
```at
// 配置 5G_LAN 路由表大小
AT+QCFG="5glan_route",50  // 50 條路由

// 配置 5G_LAN 路由快取
AT+QCFG="5glan_route",50,1  // 啟用快取

// 配置 5G_LAN 路由更新間隔
AT+QCFG="5glan_route",50,1,300  // 300 秒更新間隔
...
```at
// 配置 5G_LAN 防火牆規則
AT+QCFG="5glan_firewall","192.168.1.0/24","ALLOW"

// 配置 5G_LAN 存取控制
AT+QCFG="5glan_acl","192.168.1.100","192.168.1.200","PERMIT"

// 配置 5G_LAN 加密強度
AT+QCFG="5glan_encrypt",256  // 256 位元加密

// 配置 5G_LAN 金鑰更新
AT+QCFG="5glan_keyupdate",3600  // 3600 秒更新間隔
...
```at
// 配置 5G_LAN 頻寬管理
AT+QCFG="5glan_bandwidth",1000000  // 1Mbps 限制

// 配置 5G_LAN 延遲控制
AT+QCFG="5glan_latency",10  // 10ms 延遲要求

// 配置 5G_LAN 優先級
AT+QCFG="5glan_priority",1  // 高優先級

// 配置 5G_LAN 服務品質
AT+QCFG="5glan_qos",1,1000000,10,1
...
```at
// 配置 5G_LAN 效能監控
AT+QCFG="5glan_monitor",60  // 60 秒監控間隔

// 配置 5G_LAN 告警閾值
AT+QCFG="5glan_monitor",60,80  // 80% 使用率告警

// 配置 5G_LAN 日誌記錄
AT+QCFG="5glan_monitor",60,80,1

// 配置 5G_LAN 統計收集
AT+QCFG="5glan_stats",1  // 啟用統計收集

// 檢查 5G_LAN 監控配置
AT+QCFG="5glan_monitor"?
```
AT+QNATBUF=8192
...
// 只允許必要的入站連接
AT+QFIREWALL=1,"IN","192.168.1.100","80","ALLOW"
AT+QFIREWALL=1,"IN","192.168.1.101","443","ALLOW"

// 配置出站連接限制
AT+QFIREWALL=1,"OUT","0.0.0.0/0","80,443,53","ALLOW"
...
```at
// 配置多個外部 IP 負載均衡
AT+QNATLB=1,"203.0.113.10,203.0.113.11,203.0.113.12"

// 配置負載均衡算法 (1=輪詢, 2=最少連接, 3=加權輪詢)
AT+QNATLB=1,"203.0.113.10,203.0.113.11,203.0.113.12",2

// 配置健康檢查
AT+QNATLB=1,"203.0.113.10,203.0.113.11,203.0.113.12",2,1
